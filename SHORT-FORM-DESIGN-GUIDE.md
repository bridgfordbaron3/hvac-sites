...
A complete visual playbook for YouTube Shorts, TikTok, and Instagram Reels. Everything here is about how the video **looks** — framing, text, color, light, motion, and polish — down to the pixel. HVAC/home-services notes are called out where the niche changes the advice.
...
- Never put a CTA, phone number, or handle in the bottom 420 px. (HVAC note: if you show a phone number or "link in bio," put it center-frame on its own end card.)
...
- **Medium (waist-up):** for demonstrations, gesturing, holding a product/tool. Use as your "B" framing to cut between (see §9 punch-ins).
...
1. **Foreground** (optional): something soft/out-of-focus at a frame edge — a plant leaf, a doorframe, a tool on a bench.
...
- HVAC note: **the job site is a free production designer.** An open air handler, a furnace with the panel off, an attic with ductwork — these backgrounds carry authority no studio can. Frame the equipment over your shoulder in the background layer.
...
- Use environment lines (duct runs, copper linesets, roof edges, hallways) to point toward the subject.
...
- Start with **motion already happening**: a hand entering frame, a panel coming off, a walk-toward-camera already in progress. A static human who then starts moving reads as buffering.
...
- **Partial reveal:** show the *result* or the *problem* before context — a frosted-over coil, a burnt contactor, a $9,000 invoice — before you appear.
...
- Break lines at natural speech pauses, never mid-phrase ("your AC is / leaking money" ✓, "your AC / is leaking money" ✗).
...
- Emoji: 0–1 per caption block, only when it adds meaning (🔥 on a heat claim, 💸 on a cost claim). Emoji as bullet points in list overlays is fine. Never a row of three emoji.
...
- **Base (60%):** your dominant environment tone — usually neutral (the real world: walls, equipment, clothing).
...
- HVAC note: **blue + orange is the industry's native complementary pair** (cool/heat). Blue base + orange accent is instantly legible as HVAC, high-contrast, and colorblind-safe. If your company brand already has colors, use those — consistency beats theory.
...
- HVAC note: a clean branded polo or work shirt with a visible logo is a trust signal — keep the logo out of the caption band.
...
### Outdoors / job site (HVAC reality)
- **Open shade is your studio:** the shadow side of the house, under an eave, inside the garage with the door open. Soft, even, flattering.
- **Overcast is a gift** — the whole sky is a softbox. Shoot the talky parts on those days if you can.
- Avoid harsh noon sun on faces (hat-brim shadows, squinting, blown highlights). If unavoidable: put the sun **behind** the subject (backlight) and expose for the face; the sky blows out — acceptable.
- Attic/crawlspace/mechanical-room shots: a small on-camera LED or a headlamp aimed at the ceiling (bounce) beats direct harsh light. Embrace some grit — it's authentic — but the face must always read.
...
- **Stabilization:** locked-off tripod for talking heads; deliberate handheld with elbows tucked for walk-and-talks and job-site tours (slight motion = energy and authenticity; wobble = nausea). Use the phone's built-in stabilization; gimbals only if you already own one.
...
- Odd-numbered small rotations (±1°) or a slight reposition also sell a cut; identical-frame jump cuts read as stutters.
...
- **Pull-out** = reveal (start tight on a detail, widen to show context — perfect for before/after and "look at this whole system" moments).
...
- The three acceptable exceptions, used at most once or twice per video:
  ...
  2. **Match cut** — hand closes a panel → cut → hand opens a different panel; shapes/motion align across the cut.
...
- Timelapse for long processes (install, weld, pull-and-replace): shoot locked-off, play at 800–2000%, hold the final result at normal speed for 1.5s.
- Slow motion only from true 60fps footage, and only on moments that deserve savoring — refrigerant blow-off, a satisfying panel snap, a flame ignition sequence.
...
- Every noun you say is a b-roll prompt: say "capacitor," show the capacitor. Literal beats clever.
...
- Redact anything sensitive (customer names, addresses, phone numbers) with a solid brand-color bar — not a blur, which can be reversed and looks messier.
...
### Before/after (the HVAC money format)
...
- **Wipe:** hold the before shot 1s, wipe left-to-right (or top-to-bottom) to the after — shoot both from a **tripod at the identical position** or the effect dies. Mark tripod feet with tape on multi-hour jobs.
...
### Proof shots
- Receipts, gauges, thermostat readouts, meter readings: shoot them close, in focus, held steady 1.5s+, and boost with a subtle zoom. Numbers are the most rewatched frames — make them legible.
...
### Lower-thirds
- Use once, early (2–4s in), to say who you are: name + one-line credibility ("Baron • HVAC contractor, 12 yrs"). Slide-in from the left, hold 2.5–3s, slide out.
...
- One designed, reusable end card: solid brand background, your face or logo, **one** CTA ("Follow for more" / "Send this to someone with an old AC unit"), your handle. Center-frame, giant text, nothing in the bottom 420 px.
...
- Multi-point videos ("3 signs your furnace is dying"): a persistent small counter chip ("1/3") top-center or a thin accent-color progress bar (6–8 px) along the very top edge. Both signal "this is structured and short," which holds viewers.
- Each list item gets an identical title-card treatment (same position, animation, style) — the repetition is the design.

### Template everything
- Build one master editing project per format (talking-head explainer, job-site tour, before/after) with the safe-zone overlay, caption presets, watermark, end card, and grade already in place. Every new video starts from the template. This is the difference between 4 hours and 40 minutes per edit — and it enforces consistency automatically.
...
- **Colorblind-safe accents:** avoid red-vs-green as your only distinction (before/after labels, good/bad indicators). Blue/orange (conveniently your HVAC pair) is the most colorblind-robust complementary set. Pair color with a shape or label ("✗/✓", "BEFORE/AFTER") — never color alone.
...
- **Don't rely on tiny detail:** if the payoff is a number on a gauge, punch in until it fills a third of the frame width. Design for a 6-inch screen at arm's length in daylight — the worst realistic viewing condition.
...
- [ ] **Redaction pass:** no customer names, addresses, plates, or phone numbers visible (job-site footage!).
